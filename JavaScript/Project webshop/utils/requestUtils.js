...
const getCredentials = request => {
  ...
  if (!("authorization" in request.headers)) {
    ...
    // Split 'Basic YWRtaW5AZW1haWwuY29tOjEyMzQ1Njc4OTA=' 
    // -> [ 'Basic', 'YWRtaW5AZW1haWwuY29tOjEyMzQ1Njc4OTA=' ]
    const authHeader = request.headers["authorization"].trim();
    const splitted = authHeader.split(" ");
    const code = splitted[splitted.length - 1];
    const type = splitted[0];
    
    // Type should always be "Basic"
    if (type.toLowerCase().trim() !== "basic") {
      return null
    }

    // Decode coded part of the header
    try {
