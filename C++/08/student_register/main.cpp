...
void modifyPhoneNumber(Student *s, std::string const &new_number)
{
    s->phone_number = new_number;
}

void saveData(std::map<std::string, Student*> const &user_ids, std::string file_name)
{
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string student_num = parts.at(1);

            if (student_numbers.find(student_num) == student_numbers.end()) {
                ...
                if (!is_valid_phone_number(phone_num)) {
                    ...
                    modifyPhoneNumber(student_numbers.at(student_num), phone_num);
                }
            }
