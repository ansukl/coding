...
public:
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: Pahimmassa tapauksessa .find()-metodi sekä std::find()
    // joutuvat käymään koko tietorakenteen läpi, jolloin tehokkuus on lineaarinen.
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: Pahimmassa tapauksessa rekursiivinen funktio joutuu
    // käymään jokaisen aseman läpi.
...
private:
    ...
    struct Station {
        ...
        std::vector<TrainID> trains_through_station;
        std::vector<Station*> linked_stations;
    };
    ...
    // Tietorakenne, johon on tallennettu junan tiedot.
    struct Train {
        TrainID train_id;
        std::vector<std::pair<StationID, Time>> train_stations;
        std::vector<StationID> stations_route;
    };

    typedef std::unordered_map<StationID, Station*> Stations;
    typedef std::unordered_map<RegionID, Region*> Regions;
    typedef std::unordered_map<TrainID, Train*> Trains;
    ...
    bool recursive_function(std::vector<StationID> &path, std::vector<StationID> visited, Station* station_ptr, StationID destination);
