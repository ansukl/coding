...
Student Id: K431687
Name:       Anna Kleemola
Email:      anna.kleemola@tuni.fi

Viikko 12 Projekti

Ohjelma määrittelee muuttolaatikkoa ja sen sisältöä mallintavan luokan ja sen
metodit.
"""
...
class MovingBox:
    ...
    def add_item(self, item_name, item_count):
        """
        Lisää laatikkoon uuden esineen.
        :param item_name: str, esineen nimi
        :param item_count: int, esineiden lukumäärä
        """

        if item_name not in self.__inventory:
            self.__inventory[item_name] = item_count

        else:
            self.__inventory[item_name] += item_count

        self.__item_counter += item_count

    def list_content(self):
        """
        Tulostaa listan laatikkoon tallennetuista esineistä
        aakkosjärjestyksessä.
        """

        sorted_items = sorted(self.__inventory)

        # Tarkistetaan, onko laatikossa ollenkaan esineitä
        if self.__inventory == {}:
            print(f"Box \"{self.__box_name}\" contains 0 items.")

        else:
            print(f"Box \"{self.__box_name}\" contains {self.__item_counter} "
                  f"items.")

            # Muotoillaan tuloste niin, että lukumäärän edessä olevien
            # välilyöntien määrä riippuu lukumäärän suuruudesta
            for i in sorted_items:
                space_size = 3 - len(str(self.__inventory[i]))
                print(f"{space_size * ' '}{self.__inventory[i]} {i}")

    def remove_item(self, item_name, item_count):
        """
        Poistaa tietyn määrän esineitä nimetystä laatikosta.
        :param item_name: str, esineen nimi
        :param item_count: int, esineiden lukumäärä
        """
        ...
        # Tarkistetaan, onko annettu poistettavien esineiden lukumäärä isompi
        # kuin esineiden lukumäärä laatikossa, koska esineitä ei voi poistaa
        # määräänsä enempää
        if item_count > self.__inventory[item_name]:
            return False
    ...
    def transfer_item(self, target,  item_name, item_count):
        """
        Siirtää tietyn määrän esineitä nimetystä laatikosta toiseen.
        :param target: str, kohdelaatikon nimi
        :param item_name: str, esineen nimi
        :param item_count: int, esineiden lukumäärä
        """
        ...
        # Tarkistetaan, onko annettu siirrettävien esineiden lukumäärä isompi
        # kuin esineiden lukumäärä laatikossa, koska esineitä ei voi siirtää
        # määräänsä enempää
        elif item_count > self.__inventory[item_name]:
            return False

        else:
            target.add_item(item_name, item_count)
    ...
    def search_item(self, item_name):
        """
        Etsii ja palauttaa halutun esineen laatikosta.
        :param item_name: str, esineen nimi
        """
        ...
        # Jos esinettä ei ole laatikossa, palautetaan nolla
        else:
            return 0
